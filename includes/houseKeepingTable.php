...
?>
 ...
 <?php
        if(isset($_POST["name"])) {
          ...
          $order = $_POST["name"];
          $sql = "INSERT INTO houseKeepingArchive( SELECT * FROM houseKeeping WHERE '$order'=orderId)";
          $sql2 = "DELETE FROM houseKeeping where '$order'=orderId";        
          echo $sql;
...
<head>
  <meta charset="UTF-8">
  <title>Housekeeping table</title>
...
<table id="table" 
	...
	<th data-field="loc" data-filter-control="input" data-sortable="true">User</th>
	<th data-field="arrived" data-sortable="true">Is delivered</th>
...
echo "<tr><td>
      </td><td>". $row["orderId"]. "</td><td>". $row["name"]. "</td><td>". $row["unitsNum"] . "</td><td>".$row["user"]."</td>";
        echo "<td> <button id='".$row["orderId"]."' type='button' class='btn btn-circle btn-raised btn-success' onclick='deleteRow(this.id)' name='".$row["orderId"]."'><span class='glyphicon glyphicon-ok'></span></button></td>";
...
function deleteRow(bt) {
      ...
      var xhttp = new XMLHttpRequest();
        xhttp.open("POST", "houseKeepingTable.php", true);
