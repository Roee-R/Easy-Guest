...
?>
 ...
 <?php
        if(isset($_POST["name"])) {
          ...
          $order = $_POST["name"];
          $sql =   "INSERT INTO tuktukArchive( SELECT * FROM tuktukOrders WHERE '$order'=orderNum)";
          $sql2 = "DELETE FROM tuktukOrders where '$order'=orderNum";        
...
<table id="table" 
	...
	<th data-field="userNumber" data-filter-control="input" data-sortable="true">Guest</th>
	<th data-field="dateTime" data-filter-control="input" data-sortable="true">Date</th>
	<th data-field="loc" data-filter-control="input" data-sortable="true">Location</th>
		<th data-field="dest" data-filter-control="input" data-sortable="true">Destination</th>
	<th data-field="pass" data-filter-control="input" data-sortable="true">Passengers</th>
														<th data-field="tuktukId" data-filter-control="input" data-sortable="true">Tuktuk id</th>
													<th data-field="taxi" data-filter-control="input" data-sortable="true">Need a taxi</th>
																	<th data-field="arrived" data-sortable="true">Is arrived</th>
...
echo "<tr><td>
      </td><td>". $row["orderNum"]. "</td><td>". $row["user"]. "</td><td>". $row["DateTime"] . "</td><td>".$row["location"]."</td><td>".$row["destination"]."</td><td>".$row["numOfPass"]."</td><td>".$row["tuktuk"]."</td
        ><td>".$row["needTaxi"]."</td>";
        echo "<td> <button id='".$row["orderNum"]."' type='button' class='btn btn-circle btn-raised btn-success' onclick='deleteRow(this.id)' name='".$row["orderNum"]."'><span class='glyphicon glyphicon-ok'></span></button></td></tr>";
    }
} else {
    echo "0 results";
}
...
function deleteRow(bt) {
      var xhttp = new XMLHttpRequest();
        xhttp.open("POST", "tableTuktuk.php", true);
